...
and "alicez@leetcode.com" forward to the same email address.  (Note that this rule does not apply for 
domain names.)
...
# T: O(n * m); S: O(n) where n is number of emails and m is length of longest email string. 
    ...
    def numUniqueEmails(self, emails: List[str]) -> int:
        ...
        for email in emails:
            ...
            while ( i < len(email) and email[i] != '@' ):
                ...
                elif ( email[i] != '.' and not plusFlag ):
                    emailExtract += email[i]
                
                i += 1
            
            emailExtract += email[i:]
            unique.add(emailExtract)
