"""
Given a rectangular cake with height h and width w, and two arrays of integers horizontalCuts 
and verticalCuts where horizontalCuts[i] is the distance from the top of the rectangular cake to 
the ith horizontal cut and similarly, verticalCuts[j] is the distance from the left of the rectangular 
...
class Solution:
    def maxArea(self, h: int, w: int, horizontalCuts: List[int], verticalCuts: List[int]) -> int:
        # find the max difference between horizontalCuts 
        # find the max difference between verticalCuts
        horizontalCuts.sort()
        verticalCuts.sort()
        
        hor = h - horizontalCuts[-1]
        ver = w - verticalCuts[-1]
        
        for i in range(len(horizontalCuts)-1, 0, -1):
            hor = max(hor, horizontalCuts[i]-horizontalCuts[i-1])
        hor = max(hor, horizontalCuts[0] - 0)
        
        for j in range(len(verticalCuts)-1, 0, -1):
            ver = max(ver, verticalCuts[j]-verticalCuts[j-1])
        ver = max(ver, verticalCuts[0] - 0)
