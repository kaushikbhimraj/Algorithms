...
class Solution:
    def removeDuplicates(self, nums: List[int]) -> int:
        ...
        # Look at the previous value in the array, 
        # Case 1: If the current = previous -> then remove current and dont increment the pointer.
        # Case 2: If not -> then increment the pointer to next value. 
        # Note: Make sure to update length of array when elements are removed.
        n = len(nums)
        i = 1
        while i < n:
            if nums[i-1] == nums[i]:
