...
Each entry items[i] has items[i][0] the student's id, and items[i][1] the student's 
...
# Time:  O(students * students * log (scores))
# Space: O(students * scores)
class Solution:
    def highFive(self, items: List[List[int]]) -> List[List[int]]:
        ...
        for i in range(len(items)):
            if items[i][0] in id:
                id[items[i][0]].append(items[i][1])
            else:
                id[items[i][0]] = [items[i][1]]
        
        for key in id:
            temp = (sorted(id[key], reverse=True))[:5]
            output.append([key, sum(temp)//5])
