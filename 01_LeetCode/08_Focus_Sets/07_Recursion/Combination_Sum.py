...
class Solution:
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        sums = []
        self.sumBuilder(0, target, candidates, [], sums)
        return sums
    
    def sumBuilder(self, idx, target, candidates, curr, sums):
        if (target == 0):
            sums.append(list(curr))
            return 
        ...
        for i in range(idx, len(candidates)):
            ...
            self.sumBuilder(i, target-candidates[i], candidates, curr, sums)
            curr.pop()
        return